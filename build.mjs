...
for(const name of ['src/app.js','src/core.js','src/config.js','src/access-policy.js','src/notifications.js','src/notification-rules.js','sw.js'])execFileSync(process.execPath,['--check',path.join(root,name)]);
...
fs.writeFileSync(path.join(out,'release.json'),JSON.stringify({version:'9.24-B.1',builtAt:new Date().toISOString()}));
console.log('Build 9.24-B.1 complete: static site in dist');
