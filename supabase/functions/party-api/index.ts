...
async function notificationOperation(b:any,who:any,s:any){
 ...
 const room=scopeState(s,b.streamer||'papa').currentStreamer.id,recipient=who.role==='admin'?'__admin__':who.playerId;
 if(!recipient)throw Error('請先登入');
 const filter='streamer_id=eq.'+encodeURIComponent(room)+'&recipient=eq.'+encodeURIComponent(recipient);
 if(b.op==='notifications')return api('/rest/v1/rpc/papa_notice_inbox',{room,owner_id:recipient,page_number:Math.max(0,Math.min(100000,Math.floor(Number(b.page)||0)))});
 if(b.op==='noticeRead'){
  const ids=Array.isArray(b.ids)?b.ids.filter((id:any)=>/^[a-f0-9-]{36}$/.test(String(id))).slice(0,50):[];
  if(ids.length)await api('/rest/v1/papa_notifications?'+filter+'&id=in.('+ids.join(',')+')',{read_at:new Date().toISOString()},'PATCH');
  return {ok:true};
 }
 ...
 if(b.op==='noticeTest'){
  const [recent]=await api('/rest/v1/papa_notifications?'+filter+'&entity_id=eq.notification-test&created_at=gt.'+encodeURIComponent(new Date(Date.now()-30000).toISOString())+'&limit=1');
  if(recent)throw Error('測試通知已送出，請稍候 30 秒再試');
  const source=scopeState(s,b.streamer||'papa').currentStreamer.display_name||'主播';
  await api('/rest/v1/papa_notifications',{streamer_id:room,streamer_name:source,recipient,type:'system',level:2,entity_id:'notification-test',body:source+'｜通知測試成功 ♡'});
  schedulePush(room,[recipient]);return {ok:true};
 }
 ...
 if(b.op==='pushSubscribe'){
  ...
  if(!b.token.startsWith('player:')&&!b.token.startsWith('admin:')){
...
async function deliverPush(){
 ...
 await Promise.allSettled(jobs.map(async(job:any)=>{
  ...
  try{
   ...
   const [prefs]=await api('/rest/v1/papa_notice_preferences?streamer_id=eq.'+encodeURIComponent(n.streamer_id)+'&recipient=eq.'+encodeURIComponent(n.recipient));
   ...
   if(!sessions.length||sessions[0].player_id!==sub.recipient||n.recipient!==sub.recipient||n.read_at||!noticeChannels(prefs?.preferences,n.type).push){await api('/rest/v1/papa_push_jobs?'+jobFilter,{status:'skipped'},'PATCH');return;}
   ...
   const payload=JSON.stringify({id:n.id,title:'PA • PARTY · '+n.streamer_name,body:n.body,url:'./?streamer='+encodeURIComponent(slug)+'#'+(n.recipient==='__admin__'?'admin':'center'),recipient:n.recipient});
...
async function signalNotices(room:string,recipients:string[]){
 const prefs=await api('/rest/v1/papa_notice_preferences?streamer_id=eq.'+encodeURIComponent(room)+'&select=recipient,topic');
