...
async function api(path:string,body?:unknown,method?:string){const r=await fetch(SB_URL+path,{method:method||(body?'POST':'GET'),headers,body:body?JSON.stringify(body):undefined});const text=await r.text();if(!r.ok)throw new Error(text.includes('BOARD_RATE_LIMIT')?'留言送得太快，請稍候三秒再試':text.includes('BOARD_RETRY_CHANGED')?'重試內容不同，請重新開啟留言板':text.includes('CHAT_RATE_LIMIT')?'訊息送得太快，請稍候再送':text.includes('CHAT_REQUEST_REUSED')?'訊息重試內容不同，請重新開啟私訊':text.includes('VERSION_CONFLICT')?'資料剛更新了，請重新整理後再試一次':'資料庫操作失敗');return text?JSON.parse(text):null;}
