...
export function boardCanChange(a,p,operation){
 if(!p||!['edit','remove','restore'].includes(operation))return false;
 const owner=a.key===p.author_key,moderator=boardModerator(a,p);
 if(operation==='edit')return owner&&!p.deleted&&!p.moderated;
 return moderator||owner&&!p.moderated;
}
export function boardVisible(a,p,blocks=[]){
 if(!p)return false;
 if(boardModerator(a,p)||p.author_key===a.key)return true;
 // Filtering anonymous posts by a known account lets readers infer their author.
 if(!p.anonymous&&blocks.includes(p.author_key))return false;
...
export function boardThreadVisible(a,p,root,blocks=[]){return !!root&&boardVisible(a,root,blocks)&&(boardVisible(a,p,blocks)||root.author_key===a.key&&(p.anonymous||!blocks.includes(p.author_key)));}
export function boardProjection(a,p,authorName){
 ...
 return {id:p.id,seq:p.seq,root_id:p.root_id,body:p.deleted?'':p.body,anonymous:p.anonymous,author:p.anonymous?'匿名使用者':authorName,
  ...(p.anonymous&&reveal?{managedAuthor:authorName}:{}),created_at:p.created_at,updated_at:p.updated_at,version:p.version,deleted:p.deleted,moderated:p.moderated===true,
  visibility:p.visibility,canEdit:boardCanChange(a,p,'edit'),canManage:boardCanChange(a,p,p.deleted?'restore':'remove'),canBlock:!owner&&!moderator};
}
